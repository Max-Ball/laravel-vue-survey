  ...
  <fieldset class="mb-4">
    ...
    <div class="mt-3">
      ...
      <div v-else-if="question.type === 'radio'">
        <div v-for="(option, index) of question.data.options" :key="option.uuid" class="flex items-center">
          <input type="radio" :id="option.uuid" :name="'question' + question.id"
            @change="emits('update:modelValue', $event.target.value)" :value="option.text"
      ...
      <div v-else-if="question.type === 'checkbox'">
        <div v-for="(option, index) of question.data.options" :key="option.uuid" class="flex items-center">
          <input type="checkbox" :id="option.uuid" v-model="model[option.text]" @change="onCheckboxChange"
